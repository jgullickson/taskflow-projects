...
/**
/**
 * Adds an animation to a specified node.
 * @param {*} type 
 * @param {*} node 
 * @param {*} delay 
 */
...
/**
 * Adds an onset time to a specified node.
 */
...
/**
 * Generates a stimulus based on configuration.
 * @param {*} configuration 
 */
...
let generate_trial_block = (experiment_array, configuration, level, run_guid) => {
    ...
    trials.forEach((trial) => {
        ...
        let block = {
            ...
            boss_damage_audio: audio["BOSS_DAMAGE"],
            boss_attack_audio: audio["BOSS_ATTACK"],
        };
...
let load_level = (level, configuration, inter_session, experiment_array) => {
    ...
    if (inter_session) {
        ...
        if (level_configuration["ROOM"] !== undefined){
            document.getElementById("room-layer").style.backgroundImage = 'url('+ level_configuration["ROOM"] +')';
        }

        if (level_configuration["ROOM"] !== undefined){
            const background = new Howl({
                src: level_configuration["AUDIO"]["BACKGROUND"],
                html5: true,
                loop: true,
                volume: 1,
                onunlock() {
        ...
        if (level_configuration["WEAPON_LAYER"] !== undefined){
            document.getElementById("weapon-layer").innerHTML = level_configuration["WEAPON_LAYER"]
        }
            ...
            configuration[current_level]["BLOCK_FEEDBACK"].forEach(element => {    
                ...
                if (element["ADVANCE"] === "ADVANCE.NEUTRAL"){
                    if (
                        ...
                        // let instructionsBox = document.getElementsByClassName('instructions-box')[0];
                        // instructionsBox.innerHTML += performance_block;
