...
jsPsych.plugins["flanker-html-keyboard-response"] = (() => {
    ...
    plugin.info = {
        ...
        parameters: {
            ...
            boss_damage_audio: {
                type: jsPsych.plugins.parameterType.ARRAY,
                pretty_name: 'Boss damage audio',
                default: undefined,
                description: 'Audio (sprite) accompanying a hit to the boss (correct response).'
            }, 
            boss_attack_audio: {
                type: jsPsych.plugins.parameterType.ARRAY,
                pretty_name: 'Boss attack audio',
                default: undefined,
                description: 'Audio (sprite) accompanying an attack frimn the boss (incorrect response).'
            },               
    ...
    plugin.trial = (display_element, trial) => {
        ...
        /*-----------------------------------------------------------*/
        // weapon / boss definitions
        let active_level = parseInt(localStorage.getItem("Level"));
        let weapon = '';
        let steady = '';
        let use_weapon_left = '';
        let use_weapon_right = '';
        let boss = '';
        ...
        if (active_level === 0){
            ...
            weapon = 'fork';
            steady = 'fork-steady';
            use_weapon_left = 'fork-left';
            use_weapon_right = 'fork-right';
            ...
            weapon = 'sword';
            steady = 'steady';
            use_weapon_left = 'swing-left';
            use_weapon_right = 'swing-right';
        }
        ...
        else if (active_level === 7){
            boss = 'mummy';
            weapon = 'bow';
            steady = 'bow-steady';
            use_weapon_left = 'shoot-left';
            use_weapon_right = 'shoot-right';
        }
        else if (active_level === 8){
            boss = 'mummy';
            weapon = 'bow';
            steady = 'bow-steady';
            use_weapon_left = 'shoot-left';
        ...
        if (trial.audio_enabled) {
            var sounds = new Howl({
                ...
                sprite: {
                    ...
                    boss_damage: trial.boss_damage_audio,
                    boss_attack: trial.boss_attack_audio
        ...
        let display_feedback = (display_element, performance_label) => {
            if (performance_labels.INCORRECT === performance_label) {
                ...
                document.getElementById('hud-layer').classList.add('hurt');
                document.getElementById('boss-container').classList.add('attack');
                ...
                setTimeout(() => {
                    document.getElementById('boss').classList.add('damage');
                    document.getElementById('boss-container').classList.add('hit');
                }, 300);
        ...
        let end_trial = () => {
            ...
            //reset weapon
            document.getElementById(weapon).classList = steady

            //reset boss (damage)
            document.getElementById('boss').classList.remove('damage');
            document.getElementById('hud-layer').classList.remove('hurt');
            document.getElementById('boss-container').classList.remove('hit');
            document.getElementById('boss-container').classList.remove('attack');
            document.getElementById('boss-container').classList.remove('breathe-fire');
        ...
        let after_response = (info) => {
            ...
            //weapon behavior
                if (response.key == 37) {
                    document.getElementById(weapon).classList = use_weapon_left
                } else if (response.key == 39) {
                    document.getElementById(weapon).classList = use_weapon_right
                }
        ...
        if (trial.timely_warning_message !== undefined && trial.timely_warning_message_onset !== undefined) {
            jsPsych.pluginAPI.setTimeout(() => {
                ...
                if (boss === 'dragon'){
                    document.getElementById('boss-container').classList.add('breathe-fire')
                }
